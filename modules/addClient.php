...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql = "SELECT subscribedDavinci FROM clients WHERE (phoneNumber = ?) OR (email = ?)";
    ...
    if(count($client) < 1){ 
        ...
        //ADD NEW USER
        $sql = "INSERT INTO clients (firstName, lastName, phoneNumber, email, subscribedDavinci) VALUES (?, ?, ?, ?, ?)";
        if(!$stmt = $db->prepare($sql)){
        ...
        if(!$stmt->bind_param("ssssi",$firstName, $lastName, $phoneNumber, $email, $subscribedDavinci)){
            echo (json_encode([
                'status' => "fail",
                'message' => "Binding failed: (" . $stmt->errno . ") " . $stmt->error,
                'errors' => $errors
        ...
        if($client[0]['subscribedDavinci'] == 0){
            //ADD USER TO FORNO CLUB
            $sql = "UPDATE clients SET subscribedDavinci = ? WHERE (email = ?) OR (phoneNumber = ?)";
            if(!$stmt = $db->prepare($sql)){
            ...
            if(!$stmt->bind_param("iss",$subscribedDavinci, $email, $phoneNumber)){
                echo (json_encode([
                    'status' => "fail",
                    'message' => "Binding failed: (" . $stmt->errno . ") " . $stmt->error,
                    'errors' => $errors
